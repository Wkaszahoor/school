...
import { PlusIcon, PencilSquareIcon, TrashIcon, ArrowPathIcon, EyeIcon, MagnifyingGlassIcon, XCircleIcon } from '@heroicons/react/24/outline';
...
export default function PrincipalAcademicYearsIndex({ academicYears }: Props) {
    const [searchQuery, setSearchQuery] = useState('');

    const handleDelete = (id: number, year: string) => {
        if (confirm(`Archive academic year "${year}"? This action can be undone.`)) {
            router.delete(route('principal.academic-years.destroy', id));
        }
    };

    const handleRestore = (id: number, year: string) => {
        if (confirm(`Restore academic year "${year}"?`)) {
            router.post(route('principal.academic-years.restore', id));
        }
    };

    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('principal.academic-years.index'), { search: searchQuery });
    };

    const clearSearch = () => {
        setSearchQuery('');
        router.get(route('principal.academic-years.index'));
    };
            ...
            <div className="page-header">
                <div className="flex items-center justify-between flex-1">
                    <div>
                        ...
                        <p className="page-subtitle">View and manage academic years</p>
                    </div>
                    <Link href={route('principal.academic-years.create')} className="btn-primary">
                                        ...
                                        <td className="text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                {year.deleted_at ? (
                                                    ...
                                                    <>
                                                        <Link
                                                            href={route('principal.academic-years.show', year.id)}
                                                        ...
                                                        <Link
                                                            href={route('principal.academic-years.edit', year.id)}
